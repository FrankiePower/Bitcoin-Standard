...
/// Tests for BTSUSDVault contract
///
/// Test coverage:
/// - Deployment and initialization
/// - Collateral deposits
/// - Collateral withdrawals
/// - BTSUSD minting with collateral ratio enforcement
/// - BTSUSD burning
...
fn deploy_full_system() -> (
    ...
    IBTSUSDVaultDispatcher, // vault dispatcher
    IERC20Dispatcher, // wbtc dispatcher
    IERC20Dispatcher, // btcusd erc20 dispatcher
    IMockOracleDispatcher, // oracle dispatcher (primary)
    IMockOracleDispatcher // oracle dispatcher (secondary)
) {
    // Deploy MockWBTC
    ...
    // Deploy BTSUSDToken (with temporary vault address - will update)
    ...
    let btcusd_token = IBTSUSDTokenDispatcher { contract_address: token_address };
    ...
    (vault_address, vault, wbtc, btcusd, oracle, oracle2)
}
...
fn test_vault_deployment() {
    let (vault_address, vault, wbtc, btcusd, _, _) = deploy_full_system();
    ...
    assert(btcusd_addr == btcusd.contract_address, 'Wrong BTSUSD address');
...
fn test_deposit_collateral() {
    let (vault_address, vault, wbtc, _, _, _) = deploy_full_system();
...
fn test_mint_BTSUSD() {
    ...
    // Let's mint $30,000 BTSUSD to be safe
    let mint_amount = 30000 * ONE_BTSUSD;

    start_cheat_caller_address(vault_address, USER1());
    vault.mint_BTSUSD(mint_amount);
    stop_cheat_caller_address(vault_address);
    ...
    // Verify BTSUSD balance
    assert(btcusd.balance_of(USER1()) == mint_amount, 'Wrong BTSUSD balance');
...
fn test_mint_at_max_ltv() {
    ...
    // Mint exactly at max
    start_cheat_caller_address(vault_address, USER1());
    vault.mint_BTSUSD(max_mintable);
    stop_cheat_caller_address(vault_address);
...
fn test_cannot_mint_beyond_max_ltv() {
    ...
    let max_mintable = vault.get_max_mintable(USER1());
    let excess_amount = max_mintable + ONE_BTSUSD;

    start_cheat_caller_address(vault_address, USER1());
    vault.mint_BTSUSD(excess_amount);
    stop_cheat_caller_address(vault_address);
}
...
fn test_cannot_mint_without_collateral() {
    let (vault_address, vault, _, _, _, _) = deploy_full_system();
...
fn test_withdraw_partial_collateral_with_debt() {
    ...
    // Mint some BTSUSD
    let mint_amount = 20000 * ONE_BTSUSD; // $20,000
    start_cheat_caller_address(vault_address, USER1());
    vault.mint_BTSUSD(mint_amount);
    stop_cheat_caller_address(vault_address);
...
fn test_burn_BTSUSD() {
    ...
    // Burn half
    let burn_amount = 15000 * ONE_BTSUSD;
    start_cheat_caller_address(vault_address, USER1());
    vault.burn_BTSUSD(burn_amount);
    stop_cheat_caller_address(vault_address);
    ...
    // Verify BTSUSD balance
    assert(btcusd.balance_of(USER1()) == mint_amount - burn_amount, 'Wrong BTSUSD balance');
}
...
fn test_burn_all_debt() {
    ...
    // Burn all debt
    vault.burn_BTSUSD(mint_amount);
    stop_cheat_caller_address(vault_address);

    // Verify no debt
    let position = vault.get_position(USER1());
    assert(position.debt == 0, 'Should have no debt');
    assert(btcusd.balance_of(USER1()) == 0, 'Should have no BTSUSD');
...
fn test_cannot_burn_more_than_debt() {
    ...
    vault.burn_BTSUSD(mint_amount + ONE_BTSUSD);
    stop_cheat_caller_address(vault_address);
}
...
fn test_deposit_and_mint() {
    ...
    assert(btcusd.balance_of(USER1()) == minted, 'Wrong BTSUSD balance');
}
...
fn test_collateral_ratio_calculation() {
    ...
    let mint_amount = 32500 * ONE_BTSUSD;
    start_cheat_caller_address(vault_address, USER1());
    vault.mint_BTSUSD(mint_amount);
    stop_cheat_caller_address(vault_address);
...
fn test_is_liquidatable() {
    let (vault_address, vault, wbtc, _, oracle, oracle2) = deploy_full_system();
    ...
    oracle2.set_btc_price(5000000000000); // $50,000
