...
/// Integration Tests for BTSUSD Protocol
///
...
const ONE_BTSUSD: u256 = 1000000000000000000; // 10^18
...
struct DeployedSystem {
    ...
    oracle2_address: ContractAddress,
    oracle2: IMockOracleDispatcher,
...
fn deploy_full_system() -> DeployedSystem {
    ...
    // Deploy BTSUSDToken (with temporary vault address)
    ...
    let btcusd_token = IBTSUSDTokenDispatcher { contract_address: btcusd_address };
...
fn test_full_deposit_mint_burn_withdraw_cycle() {
    ...
    // Step 2: Mint BTSUSD at safe ratio
    let mint_amount = 30000 * ONE_BTSUSD;
    user_mint(@sys, USER1(), mint_amount);

    assert(sys.btcusd.balance_of(USER1()) == mint_amount, 'BTSUSD not minted');
    assert(sys.vault.get_position(USER1()).debt == mint_amount, 'Debt not recorded');

    // Step 3: Burn all BTSUSD
    user_burn(@sys, USER1(), mint_amount);

    assert(sys.btcusd.balance_of(USER1()) == 0, 'BTSUSD not burned');
...
fn test_deposit_and_mint_convenience_function() {
    ...
    assert(sys.btcusd.balance_of(USER1()) == minted, 'Wrong BTSUSD balance');
}
...
fn test_multiple_users_independent_positions() {
    ...
    user_deposit(@sys, USER1(), ONE_WBTC);
    user_mint(@sys, USER1(), 25000 * ONE_BTSUSD);

    // User2 deposits more and mints more
    user_deposit(@sys, USER2(), 2 * ONE_WBTC);
    user_mint(@sys, USER2(), 50000 * ONE_BTSUSD);
    ...
    assert(pos1.debt == 25000 * ONE_BTSUSD, 'Wrong USER1 debt');

    assert(pos2.collateral == 2 * ONE_WBTC, 'Wrong USER2 collateral');
    assert(pos2.debt == 50000 * ONE_BTSUSD, 'Wrong USER2 debt');
    ...
    assert(total_collateral == ONE_WBTC + 2 * ONE_WBTC + ONE_WBTC / 2, 'Wrong total collateral');
    assert(total_debt == 25000 * ONE_BTSUSD + 50000 * ONE_BTSUSD, 'Wrong total debt');
}
...
fn test_btcusd_transfers_between_users() {
    ...
    // User1 gets BTSUSD
    user_deposit(@sys, USER1(), ONE_WBTC);
    user_mint(@sys, USER1(), 30000 * ONE_BTSUSD);

    // Transfer to User2
    let transfer_amount = 10000 * ONE_BTSUSD;
    ...
    // Verify balances
    assert(sys.btcusd.balance_of(USER1()) == 20000 * ONE_BTSUSD, 'Wrong USER1 balance');
    assert(sys.btcusd.balance_of(USER2()) == transfer_amount, 'Wrong USER2 balance');
    // User2 can now deposit collateral and use received BTSUSD to start a position
...
fn test_price_increase_improves_health() {
    ...
    start_cheat_caller_address(sys.oracle2_address, OWNER());
    sys.oracle2.set_btc_price(BTC_PRICE_100K);
    stop_cheat_caller_address(sys.oracle2_address);
...
fn test_price_decrease_worsens_health() {
    ...
    stop_cheat_caller_address(sys.oracle_address);
    start_cheat_caller_address(sys.oracle2_address, OWNER());
    sys.oracle2.set_btc_price(BTC_PRICE_50K);
    stop_cheat_caller_address(sys.oracle2_address);
...
fn test_severe_price_drop_makes_liquidatable() {
    ...
    stop_cheat_caller_address(sys.oracle_address);
    start_cheat_caller_address(sys.oracle2_address, OWNER());
    sys.oracle2.set_btc_price(4500000000000); // $45,000
    stop_cheat_caller_address(sys.oracle2_address);
...
fn test_very_small_mint() {
    ...
    // Mint just 1 BTSUSD
    user_mint(@sys, USER1(), ONE_BTSUSD);

    let position = sys.vault.get_position(USER1());
    assert(position.debt == ONE_BTSUSD, 'Small mint failed');
}
...
fn test_partial_operations() {
    ...
    // Mint some BTSUSD
    user_mint(@sys, USER1(), 40000 * ONE_BTSUSD);

    // Burn partial
    user_burn(@sys, USER1(), 10000 * ONE_BTSUSD);
    ...
    let position = sys.vault.get_position(USER1());
    assert(position.debt == 30000 * ONE_BTSUSD, 'Wrong remaining debt');
}
...
fn test_protocol_stats_accurate() {
    ...
    user_deposit(@sys, USER1(), ONE_WBTC);
    user_mint(@sys, USER1(), 20000 * ONE_BTSUSD);

    user_deposit(@sys, USER2(), 2 * ONE_WBTC);
    user_mint(@sys, USER2(), 40000 * ONE_BTSUSD);

    let (total_collateral, total_debt) = sys.vault.get_protocol_stats();
    assert(total_collateral == 3 * ONE_WBTC, 'Wrong total collateral');
    assert(total_debt == 60000 * ONE_BTSUSD, 'Wrong total debt');

    // User1 repays and withdraws
    user_burn(@sys, USER1(), 20000 * ONE_BTSUSD);
    user_withdraw(@sys, USER1(), ONE_WBTC);

    let (new_collateral, new_debt) = sys.vault.get_protocol_stats();
    assert(new_collateral == 2 * ONE_WBTC, 'Wrong new collateral');
    assert(new_debt == 40000 * ONE_BTSUSD, 'Wrong new debt');
}
