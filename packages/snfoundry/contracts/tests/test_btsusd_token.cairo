use contracts::interfaces::{IBTSUSDTokenDispatcher, IBTSUSDTokenDispatcherTrait};
use openzeppelin_token::erc20::interface::{IERC20Dispatcher, IERC20DispatcherTrait};
/// Tests for BTSUSDToken contract
///
/// Test coverage:
...
fn deploy_btsusd_token() -> (ContractAddress, IBTSUSDTokenDispatcher, IERC20Dispatcher) {
    let contract = declare("BTSUSDToken").unwrap().contract_class();
    ...
    let token_dispatcher = IBTSUSDTokenDispatcher { contract_address };
...
fn test_deployment_initializes_correctly() {
    let (_, token, erc20) = deploy_btsusd_token();
...
fn test_vault_can_mint() {
    let (contract_address, token, erc20) = deploy_btsusd_token();

    let mint_amount: u256 = 1000_000000000000000000; // 1000 BTSUSD
...
#[test]
#[should_panic(expected: 'BTSUSD: caller is not vault')]
fn test_non_vault_cannot_mint() {
    let (contract_address, token, _) = deploy_btsusd_token();
...
#[test]
#[should_panic(expected: 'BTSUSD: zero address')]
...
#[test]
#[should_panic(expected: 'BTSUSD: zero amount')]
