...
///
/// Provides a simplified interface for the BTSUSD Protocol to interact with
...
/// 3. Once confirmed, user/relayer calls `claim_deposit`
...
pub trait IAtomiqAdapter<TContractState> {
    ...
    fn claim_deposit(
        ref self: TContractState, deposit_id: u256, btc_tx_hash: u256, merkle_proof: Span<u256>,
    );

    /// Initiate a wBTC → BTC withdrawal
    fn initiate_withdrawal(
        ref self: TContractState, amount: u256, btc_address_hash: felt252,
    ) -> u256;
...
pub mod AtomiqAdapter {
    use contracts::interfaces::{IBtcRelayDispatcher, IBtcRelayDispatcherTrait};
    ...
    const DEFAULT_DEPOSIT_EXPIRY: u64 = 86400; // 24 hours
    ...
    impl AtomiqAdapterImpl of IAtomiqAdapter<ContractState> {
        fn request_deposit(ref self: ContractState, amount_sats: u64) -> (u256, felt252) {
            ...
            // Deterministic BTC address hash derived from user + deposit_id
            ...
            self
                .emit(
                    DepositRequested {
                        deposit_id, user: caller, amount_sats, btc_address_hash, expires_at,
                    },
                );
        ...
        fn claim_deposit(
            ref self: ContractState, deposit_id: u256, btc_tx_hash: u256, merkle_proof: Span<u256>,
        ) {
            self.pausable.assert_not_paused();
            ...
            let btc_relay = IBtcRelayDispatcher { contract_address: self.btc_relay.read() };
            let required_confirmations = btc_relay.get_required_confirmations();
            let latest_height = btc_relay.get_latest_block_height();

            let is_valid = btc_relay
                .verify_tx_inclusion(
                    btc_tx_hash, latest_height - required_confirmations, merkle_proof, 0,
                );
            assert(is_valid, 'BTC tx not verified');

            // Satoshis map 1:1 to wBTC base units (8 decimals)
            ...
            self
                .emit(
                    DepositClaimed {
                        deposit_id, user: deposit.user, amount: wbtc_amount, btc_tx_hash,
                    },
                );
        }

        fn initiate_withdrawal(
            ref self: ContractState, amount: u256, btc_address_hash: felt252,
        ) -> u256 {
            self.pausable.assert_not_paused();
            ...
            self
                .emit(
                    WithdrawalInitiated { withdrawal_id, user: caller, amount, btc_address_hash },
                );
        ...
        fn get_user_deposits(self: @ContractState, user: ContractAddress) -> Span<u256> {
            ...
            let mut i: u256 = 0;
            while i < count {
                result.append(self.user_deposits.read((user, i)));
                i += 1;
            }
            result.span()
        }
        ...
        fn get_user_withdrawals(self: @ContractState, user: ContractAddress) -> Span<u256> {
            ...
            let mut i: u256 = 0;
            while i < count {
                result.append(self.user_withdrawals.read((user, i)));
                i += 1;
            }
            result.span()
        }
